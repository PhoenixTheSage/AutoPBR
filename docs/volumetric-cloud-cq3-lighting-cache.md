...
**Status:** CQ3 through CQ3.9 complete and accepted
...
- [x] CQ3.8a: Implement continuous-altitude shell stabilization. Completed 2026-07-29 with stable implicit sphere/root math, separate zero-density support bounds, ray-distance-anchored primary sampling, profiled cirrus path integration, continuous signed-altitude fault diagnostics, CPU/source coverage, and live generic/High/repair shader compilation.
- [x] CQ3.8b: Accept animated boundary sweeps and confirm the CQ3.7 performance/depth gates remain intact before CQ4.0. Completed 2026-07-29 with 16 live High/Cinematic, temporal-on/off cumulus/cirrus boundary sweeps and a passing frozen 1080p High trace gate (`0.617/0.718 ms` p50/p95 across 240 samples).
- [x] CQ3.9a: Flatten cloud geometry for the continuous world. Completed 2026-07-30: trace, repair, density, light-cache generation, light-cache bounds, CPU reference math, and reconstruction share vertical `worldY - groundY` altitude; the obsolete planet mask, curvature allowance, and camera-altitude opacity multiplier are removed.
- [x] CQ3.9b: Complete live High/Cinematic boundary, long-distance/depth, visual, and performance acceptance. Completed 2026-07-30 with all 16 High/Cinematic, temporal-on/off, cumulus/cirrus boundary sweeps passing after the harness was corrected to retain legitimate clear-sky frames. A fresh 13-case `1920×1080` matrix retained 3,120 GPU samples and covered below/inside/above-layer, grazing-distance, cirrus, terrain/depth ordering, moving shadows, and sun transitions. Frozen High dense-overcast measured `0.514/0.547 ms` trace p50/p95 and `0.514 ms` amortized lighting, or `0.931×` CQ2 versus the `1.25×` gate.
...
- `PreviewCloudLightingShadingProfiles` freezes the two higher-order controls as `(extinction, phase-eccentricity, energy)`: octave one is `(0.50, 0.50, 0.55)` and octave two is `(0.25, 0.25, 0.30)`. The profile also owns the `2.25` post-sum energy clamp. CA3.3 later retuned the companion contrast controls to a `0.14` cached-sky floor, `0.13` ground-bounce energy, and `0.38` local-cone optical-depth scale (CQ3.4 originally shipped `0.18` / `0.11` / `0.45`).
...
- The former `PreviewCloudCameraRegion` Below/Inside/Above value selected no renderer, cache, or temporal path; it existed only as transition telemetry. CQ3.8 removes that discrete state and records continuous radial altitude plus signed distances to both cumulus and cirrus boundaries only when a contained render failure needs diagnostics.
- Shell altitude and density evaluation now derive vertical height from `worldY - groundY` before introducing the 72,000-unit radius. Rationalized altitude and implicit-sphere equations avoid the former 0.0078125-unit cancellation grid, while a stable `q/cq` quadratic formulation protects near roots.
- Cumulus and cirrus intersection support extends into bounded zero-density guard regions. The physical density bounds remain unchanged, so shell topology changes while contribution is already zero.
- The primary cumulus sample phase is anchored to ray distance rather than `tEnter`; centimeter-scale grazing-boundary motion no longer relocates the complete sample lattice.
- Cirrus now has a smooth vertical envelope and integrates actual profiled path length. The former minimum-one slant clamp, which assigned full minimum optical depth to an arbitrarily short segment and then dropped directly to zero, is removed from trace and repair.
- Focused stable-altitude/root/source tests pass, and the explicitly enabled hidden-WGL smoke compiles generic, specialized High, temporal, upsample, and repair programs.
- CQ3.8b's render-readiness-aware hidden-WGL matrix moves the camera in 0.2-unit increments through cumulus base/top and cirrus base/top. All 16 High/Cinematic and temporal-on/off combinations pass the adjacent-frame delta gate without a contained cloud fault. A startup-black false positive found during bring-up is now excluded by waiting for a nonzero rendered scene and settling temporal state before measurement.
- The first frozen High performance rerun exposed a `1.039 ms` p50 regression from exact rationalized altitude evaluation in every density query. Density-only altitude now uses a stable third-order radius expansion (maximum double-precision reference deviation `0.000039` units over the tested `-2..180` altitude and `0..5,500` horizontal domain), while intersections and diagnostics remain exact. Together with two profiled High cirrus taps and four Cinematic taps, the accepted 1080p High result is `0.617/0.718 ms` trace p50/p95 over 240 samples, below CQ3.7's `0.671 ms` p50 and the `0.690 ms` gate.

### CQ3.9 flat continuous-world layers

- CQ3.8's continuous signed-altitude behavior remains, but cloud altitude is now exactly `worldY - groundY` at every XZ position. The old 72,000-unit preview-planet radius survives only as a temporary uniform ABI value whose center-plus-radius sum reconstructs the flat ground datum for compatibility density calls.
- Cumulus and cirrus use bounded horizontal slab intersections. Rays originating inside a layer can travel horizontally, while rays outside it must point toward the layer. All intervals terminate at 4,096 world units.
- A smooth visibility fade applies only when the slab entry lies in the last 20 percent of that finite trace range. This replaces the spherical tangent/horizon cutoff without curving the deck or introducing a hard terminal rim.
- Flat slabs have a genuine ray-topology change when a horizontal camera crosses a boundary. The initial CQ3.9a implementation tried to conceal it with a symmetric camera-boundary opacity crossfade. User runtime screenshots on 2026-07-30 proved that reaching zero at cumulus base/top and cirrus base/top recreated the obsolete altitude bands even though no renderer mode switch remained. That extinction path is removed from trace, repair, and final reconstruction. Continuity now comes from zero-density support guards, smooth vertical density envelopes, ray-distance-anchored sampling, and temporal rejection; camera altitude never directly scales cloud opacity or radiance.
- Inside-layer near-horizontal views exposed a second continuity failure: coverage probes and primary march steps sized themselves from the full altitude-plane exit, which flips between floor and ceiling across eye height and can stretch to the 4,096-unit trace cap. The first correction fixed the depth fighting by counting only real density samples, probing near-field plus full-interval coverage, and preventing clear-shell rays from publishing the floor/ceiling exit as valid nearest-cloud metadata, but its `inside` march branch selected a visibly different integrator at the slab boundary. The reconciled policy is interval-driven instead: short intervals divide their actual span, while all long/grazing intervals use the same bounded near span and continuous local-distance growth regardless of camera region. Camera altitude still never selects a marcher or scales opacity.
- Dense camera-inside paths retain a conservative diffuse-sky radiance floor after direct sunlight is extinguished, preventing long flat-layer optical paths from collapsing to unlit black.
- Opaque scene depth is still a hard upper bound in trace and repair. Full-resolution reconstruction retains per-tap cloud-distance versus scene-depth rejection and no longer applies a second planet-side mask.
- CQ3 light-cache depth bounds cover the flat vertical cloud envelope and filter guard only; they no longer grow with cascade footprint to accommodate curvature. Light-cache density and short local light marches use the same flat altitude function as the view marcher.
- CPU reference coverage proves XZ-invariant altitude, large-coordinate centimeter stability, horizontal inside/outside behavior, bounded distance fade, scene occlusion, equal step sizing for equal intervals on either side of a boundary, and span-limited march sizing for long level views. Source-contract tests cover the absence of planet intersection/mask logic, prohibit camera-boundary extinction or layer-height uniforms in final reconstruction, and prohibit an `insideSlabMarch` integrator branch.
- CQ3.9b acceptance uses `AUTOPBR_RUN_CQ39_ACCEPTANCE=1` for the 16 High/Cinematic temporal-on/off boundary combinations. Its render-readiness rule rejects only near-black startup captures: a uniform clear sky is valid for a horizontal ray outside a flat slab and must remain in the delta sequence. All 16 combinations pass.
- The final desktop GL `4.6.0 NVIDIA 610.74` / RTX 2080 Ti acceptance reran all 13 `1920×1080` CQ3 visual fixtures with 32 warm-up frames and 240 retained samples per case. Below/inside/above, grazing near/far, cirrus, terrain-shadow, depth ordering, and sun-transition captures pass. Frozen High dense-overcast records `0.514/0.547 ms` trace p50/p95, `0.701/1.511 ms` cloud-total p50/p95, no scheduled static-cache generation, and a `0.931×` CQ2 lighting ratio below the `1.25×` ceiling. Evidence is under `artifacts/volumetric-cloud-cq39-acceptance/`.
