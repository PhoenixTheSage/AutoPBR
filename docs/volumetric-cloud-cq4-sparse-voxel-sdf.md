...
Physical brick index 4095 is reserved for a valid fallback/zero brick only if required by the implementation; otherwise allocator capacity is 4,095 mapped bricks because zero remains the empty sentinel. The chosen convention must be fixed in shared CPU/shader constants and tested at both boundaries.
...
- [ ] CQ4.4: Add compute brick generation, border filling, conservative distance, barriers, and fences.
- [ ] CQ4.5: Add page DDA, SDF stepping, CQ2 detail evaluation, and cascade/shell blending.
- [ ] CQ4.6: Integrate sparse density with CQ1 metadata/temporal behavior and CQ3 cache invalidation.
- [ ] CQ4.7: Add overflow/fault recovery, debug views, counters, and GPU timings.
- [ ] CQ4.8: Complete fly-through, residency, visual, memory, fallback, and performance acceptance.
